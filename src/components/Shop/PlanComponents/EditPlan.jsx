...
const EditPlan = () => {
  ...
  useEffect(() => {
    if (plan?.features) {
      // Format features with <br> for ReactQuill
      const formattedFeatures = plan.features.join("<br>");
      setEditorContent(formattedFeatures);
    }
  }, [plan]);
  ...
  return (
    <section className="overflow-y-auto h-screen custom-scrollbar">
      <div className="w-full h-fit border border-[#E1E5E8] rounded-[30px] bg-white">
        <h1 className="mt-[25px] ml-10 font-inter font-[600] text-[24px] leading-[36px]">
          Edit Plan
        </h1>

        {/* Text Editor Section */}
        <div className="w-auto h-[300px] mt-4 mx-6 bg-[#FCFCFD] border-2 border-[#FCFCFD] rounded-md">
          <ReactQuill
            value={editorContent}
            onChange={handleEditorChange}
            className="h-[260px] bg-[#FCFCFD] rounded-md"
        ...
        <div className="flex justify-between mt-10 mx-6">
          <div>
            ...
            <input
              id="price"
              type="number"
              defaultValue={plan?.title?.match(/\d+\.?\d*/)} // Extract price from title
